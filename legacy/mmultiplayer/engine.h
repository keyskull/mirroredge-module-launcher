...
bool RunConsoleCommandNow(const wchar_t *command);
...
bool IsModReady();
void MarkReady();
bool IsGameReadyForModInit();
using MainThreadTask = void (*)();
void SetDeferredInitCallback(MainThreadTask initCallback);
void SetHostedMode(bool hosted);
bool IsHostedMode();
void SetHostedGameplayLive(bool live);
bool IsHostedGameplayLive();
bool InstallRendererCapture();
bool IsModD3D9ProxyActive();
bool HookDirect3D9Interface(IDirect3D9 *d3d);
void OnProxyDeviceCreated(IDirect3DDevice9 *device);
bool TryCaptureRenderer();
bool ArePresentationHooksInstalled();
bool InstallPeekMessageBootstrap();
...
bool TryInstallGameplayHooksSync();
bool TryInstallGameplayHooksHosted();
bool InstallPresentationHooks(IDirect3DDevice9 *device = nullptr);
