...
- **Patch** copies launcher-local `modules\` + `d3d9.dll` into the game root / `Binaries` (no launcher alias/bat). **Update modules** copies only `modules\` (+ `VERSION.json`). **Fix PhysX** / **Patch** / **Update modules** run on a background thread (UI busy lock) and resolve a local PhysX source (`physx\` pack → NVIDIA install → game `Binaries`), overlays onto `Binaries\` (see [`physx/README.md`](../physx/README.md)); if none are complete it downloads the official NVIDIA PhysX System Software installer.
