...
public:
	...
	unsigned long                                      PropertySize;                                             // 0x004C(0x0004)
	TArray<unsigned char>                              Script;                                                   // 0x0050(0x000C) — bytecode
	unsigned char                                      UnknownData00[0x34];                                     // 0x005C(0x0034)

	// Safe bytecode access — returns first byte or nullptr if Script is empty.
	const unsigned char *ScriptData() const { return Script.Buffer();  }
	size_t               ScriptSize() const { return Script.Num();     }
	bool                 HasScript()   const { return Script.Num() > 0; }
