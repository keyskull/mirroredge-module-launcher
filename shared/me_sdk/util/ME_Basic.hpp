...
struct FName
{
		  ...
		  Number(0)
	{
		...
		for (auto i : cache)
		{
			if (!GetGlobalNames().IsValidIndex(i))
			{
				continue;
			}

			const auto *entry = GetGlobalNames()[i];
			if (entry && entry->GetName() == nameToFind)
			{
				Index = i;
	...
	{
		if (!GNames || !GetGlobalNames().IsValidIndex(static_cast<size_t>(Index)))
		{
			return std::string();
		}

		const auto *entry = GetGlobalNames()[static_cast<size_t>(Index)];
		if (!entry)
		{
			return std::string();
		}
