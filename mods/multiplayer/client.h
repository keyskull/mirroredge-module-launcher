...
#pragma pack(push, 1)
    typedef struct {
        ...
        // Trailing UU/s (phase4). Old 676-byte peers omit this; recv path
        // treats missing trailer as derive-from-delta.
        Classes::FVector Velocity;
        // Phase B3-lite: TdPawn MovementState + Physics (optional after 688).
...
#pragma pack(pop)
    ...
    class Player {
      public:
        ...
        PACKET FromPacket;
        PACKET ToPacket;
        unsigned long long FromTime = 0;
        unsigned long long ToTime = 0;
        // True only after a UDP packet with non-zero compressed bones.
        // Harness bots send location/yaw with all-zero bones; applying those
        // via TransformBones collapses the mesh to an invisible point.
        bool HasRemoteBoneMotion = false;
        Classes::FVector RemoteVelocity = {};
        bool HasRemoteVelocity = false;
        int RemoteMovementState = 0;
        int RemotePhysics = 0;
        bool HasRemoteMoveState = false;
        // Post-lerp bone EMA (compressed float slots); reset on bigJump / leave.
        float BoneSmooth[ARRAYSIZE(CompressedBoneOffsets)] = {};
        bool HasBoneSmooth = false;
        // After first live (non-rest) UDP bones: boost/snap EMA for a few frames
        // to avoid rest→walk T-pose flash (mesh visual V2).
        int BoneFirstLiveBoostFrames = 0;
        // TCP interact "wave": face-host yaw window only (Mesh V13). No bone
        // float nudges — remotes use host Mesh3p / TransformBones only.
        int WaveGestureFrames = 0;
        // Last walking/idle bone pose — used when parkour state lingers after
        // landing (SoftProbe FallDrop / worldClamp floor) so remotes do not
        // keep fall limbs while Z is glued to the ground (mesh visual V4).
        PACKET LastGoodBonePose = {};
        bool HasLastGoodBones = false;
        // Grounded-parkour override only after a real fast fall (|Vz| spike).
        // SoftProbe FallDrop hovers at host+1000 with V≈0 + MOVE_Falling — that
        // used to trip the override and snap remotes back to join/rest bones.
        unsigned long long LastFastFallMs = 0;
        // V16: Idle/Walking/Falling bucket crossfade for live UDP remotes.
        int ClipBucket = 0;
        bool HasClipBucket = false;
        // V16b: hold pending stance before commit (Idle↔Walk chatter).
        int PendingClipBucket = 0;
        unsigned long long PendingClipBucketSinceMs = 0;
        bool HasBoneXfade = false;
        unsigned long long BoneXfadeStartMs = 0;
        PACKET BoneXfadeFrom = {};
        PACKET LastRenderedBones = {};
        bool HasLastRenderedBones = false;
        // V19: per-remote adaptive interp (UDP interval + jitter peak).
    ...
    struct HarnessStatus {
        ...
        // Remotes with Actor + ToTime>0 (UDP pose driving location).
        int posedPlayers = 0;
        float posX = 0.f;
        float posY = 0.f;
        float posZ = 0.f;
    ...
    static bool GetHarnessStatus(HarnessStatus &out);
    static void RefreshHarnessPose();
};
